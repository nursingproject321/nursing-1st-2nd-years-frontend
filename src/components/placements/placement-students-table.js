...
const PlacementStudentsTable = forwardRef(({ data = {} }, ref) => {
  const { placementStore, placementLocationStore } = useStore();
  ...
  const [availableSeatsMap, setAvailableSeatsMap] = useState(() => {
    ...
    placementLocations.forEach((location) => {
      ...
      const filledSeats = placements.filter(
        (placement) => location._id === placement.placementLocationId
      ).length;
  ...
  const placementMap = useMemo(
    () =>
      placements.reduce((obj, placement) => {
  ...
  const studentsPlacements = useMemo(
    () =>
      data.students.map((student) => ({
        ...student,
        name: `${student.fname} ${student.lname}`,
        placementLocationId:
          placementMap[student._id]?.placementLocationId || "",
  ...
  const [locationsListForSelectBox, setLocationsListForSelectBox] = useState(
    getLocations()
  );

  const handleLocationChange = useCallback(
    (studentId, newLocationId) => {
      const rowIndex = studentsPlacements.findIndex(
        (tableRow) => tableRow.studentId === studentId
      );
      ...
      if (newLocationId === "" || availableSeatsMap[newLocationId] > 0) {
        ...
        // set in table data
        const tableRowIndex = tableData.findIndex(
          (tableRow) => tableRow.studentId === studentId
        );
  ...
  const handleReset = useCallback(() => {
    ShowConfirmDialog({
      title: "Reset Changes",
      description:
        "Are you sure you want to reset the changes? Any unsaved changes will be lost.",
      actionBtnName: "Reset",
    ...
    ShowDialog({
      ...
      onConfirm: () => {
        ...
        Object.keys(map).forEach((key) => {
          if (key && map[key] > 0) {
            placementLocationStore.edit(key, { seats: map[key] });
          }
        });
      ...
      },
      {
        ...
        options: {
          ...
          customBodyRender: (value, tableMeta) => {
                ...
                <SelectBox
                  ...
                  onChange={(val) =>
                    handleLocationChange(
                      tableMeta.currentTableData[tableMeta.rowIndex]?.data[1] ||
                        "",
                      val
                    )
                  }
            ...
            return (
              locationsListForSelectBox.find(
