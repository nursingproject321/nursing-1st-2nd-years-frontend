...
import ShowSnackbarAlert, {
  ShowErrorAlert,
  ShowSuccessAlert,
} from "../common/SnackBarAlert";
...
function Students() {
        ...
        <>
          <Alert severity="warning" sx={{ my: 1 }}>
            <Typography variant="subtitle1">
              ...
              Once Students are moved to the next Term, you can't move them
              back.
            </Typography>
          </Alert>
          <Typography variant="subtitle2" color="error">
            Are you sure you want to move all students to the next term? Click
            Move to confirm.
          </Typography>
        </>
      ),
  ...
  const getToolBarActions = useCallback(
    () => (
      <Box>
        ...
        <Button startIcon={<GroupsIcon />} onClick={handleMoveStudentsClick}>
          Move Students
        </Button>
      </Box>
    ),
            ...
            <Box sx={{ display: "flex", gap: "1rem" }}>
              ...
              <Tooltip title="Delete" arrow placement="right">
                <DeleteIcon
                  ...
                  sx={{ cursor: "pointer" }}
                  onClick={(event) =>
                    handleDeleteRow(tableMeta.rowIndex, event)
                  }
                />
              </Tooltip>
            </Box>
  ...
  const tableData = toJS(list).slice(
    currentPage * FETCH_LIMIT,
    currentPage * FETCH_LIMIT + FETCH_LIMIT
  );
  const isFilterValueExist =
    Object.values(filters).filter((val) => val).length > 0;

  return (
    <Box
      className="table-container"
      sx={{ overflowX: "auto", width: "calc(100vw - 300px)" }}
    >
      <Table
        key={`${Date.now()}`}
        data={tableData}
        ...
        options={{
          ...
          //     superSearch(searchText.toLowerCase(), { ...row }).numberOfMatches >
          //     0,
          tableBodyHeight: isFilterValueExist
            ? "calc(100vh - 215px)"
            : "calc(100vh - 180px)",
          ...
          onSearchChange: (searchQuery, currentRow, columns) => {
            console.log("====================================");
            console.log("data", { searchQuery, currentRow, columns });
            console.log("====================================");
          },
          center: true,
        }}
