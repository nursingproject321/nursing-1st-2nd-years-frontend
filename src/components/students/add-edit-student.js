...
export default function AddEditStudent() {
  ...
  const schoolList = useMemo(
    () =>
      schoolStore.list.slice().map((school) => ({
  ...
  const isFlaggedRef = useRef(null);
  ...
  const handleSubmit = useCallback(async () => {
    ...
    const isFlagged = isFlaggedRef.current.getSelectedValue();
    const notes = notesRef.current.value();

    const isFNameValid = validate(
      fname,
      fnameRef.current,
      "Please enter the First name"
    );
    const isLNameValid = validate(
      lname,
      lnameRef.current,
      "Please enter the Last name"
    );
    const isStudentIdValid = validate(
      studentId,
    ...
    const isstudyYearValid = validate(
      year,
    ...
    const isTermValid = validate(
      term,
      termRef.current,
      "Please select the term"
    );

    const isFlaggedValid = validate(
      isFlagged,
    ...
    const params = {
      ...
      study_year,
      term,
      isFlagged: isFlagged == "Yes" ? true : false,
      year,
      notes,
    };
    try {
    ...
    return (
      <Box sx={{ mb: 2 }}>
        <LoadingButton
          ...
          sx={{ mr: 1 }}
        />
        <Button onClick={goBack} color="grey" variant="outlined">
          Cancel
        </Button>
      </Box>
    );
  ...
  useEffect(() => {
    GlobalEventEmitter.emit(EVENTS.UPDATE_TOP_BAR, {
      text: editObj ? (
        <>
          Student -<em>{`${editObj.fname} ${editObj.lname}`}</em>
        </>
      ) : (
        "Add New Student"
      ),
